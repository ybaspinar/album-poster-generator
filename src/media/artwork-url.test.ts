...
describe("createExportableArtworkUrl", () => {
  ...
  it("upgrades HTTP artwork URLs to HTTPS before downloading", async () => {
    const blob = new Blob(["image"], { type: "image/jpeg" });
    const fetcher = vi.fn().mockResolvedValue(new Response(blob, { status: 200 }));
    const createObjectUrl = vi.fn().mockReturnValue("blob:https-artwork");

    await expect(
      createExportableArtworkUrl("http://dn721907.ca.archive.org/cover.jpg", {
        fetcher,
        createObjectUrl,
      }),
    ).resolves.toEqual({
      ok: true,
      artworkUrl: "blob:https-artwork",
    });
