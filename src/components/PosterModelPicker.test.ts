...
describe("PosterModelPicker", () => {
  it("renders the creator models as album preview cards", () => {
    const wrapper = mount(PosterModelPicker, {
      props: { draft, selectedModelId: "standard" },
    });

    expect(wrapper.text()).toContain("Choose a poster model");
    expect(wrapper.findAll('[data-test^="poster-model-"]')).toHaveLength(4);
    expect(wrapper.findAll("[data-export-poster]")).toHaveLength(4);
    expect(wrapper.find('[data-test="poster-model-standard"]').text()).toContain("Starboy");
    expect(
      wrapper.find('[data-test="poster-model-standard"]').find(".poster-art").attributes("src"),
    ).toBe("https://example.com/starboy.jpg");
    expect(
      wrapper.find('[data-test="poster-model-full-cover"]').find(".poster-page").classes(),
    ).toContain("poster-layout-edge-to-edge");
  });
